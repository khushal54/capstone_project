...
commonApp.put("/password",verifyToken("USER","AUTHOR","ADMIN"),async(req,res)=>{
  ...
  if (currentPassword===newPassword){
    return res.status(400).json({message:"Current password and new password should not be the same"});
  }
  
  //Get user from req.user (populated by verifyToken)
  ...
  if (!isMatched) {
    return res.status(400).json({ message:"Current password must be same as original password" });
  }
